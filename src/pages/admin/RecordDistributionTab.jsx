  ...
  return (
    <div className="min-h-screen bg-cream px-4 py-8">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mb-6 text-xs text-faint">Locked until the family head confirms with a thumbmark</p>
        <AdminTabs />

        {/* Everything below is wrapped to look like a single official FACED document */}
        <div className="overflow-hidden rounded-lg border-2 border-navy bg-white">
          ...
          <Card className="rounded-none border-0 border-b border-line p-6 shadow-none">
            <SectionHeader>Serial number</SectionHeader>
            <p className="mb-3 text-xs text-faint">
              Check first if this beneficiary already has a record. If yes, you won't need to re-fill the whole form.
            </p>
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                placeholder="Enter serial number"
                value={serialNumber}
                onChange={(e) => { setSerialNumber(e.target.value); setSerialChecked(false); setExistingRecord(null) }}
                className={inputClass}
              />
              <button
                onClick={checkSerialNumber}
                disabled={!serialNumber.trim() || checkingSerial}
                className="whitespace-nowrap rounded bg-admin px-5 py-2 text-sm font-bold text-white disabled:opacity-50"
              >
                {checkingSerial ? 'Checking…' : 'Check serial number'}
              </button>
            </div>

            {serialChecked && existingRecord && (
              <div className="mt-4 rounded border border-warn-text bg-warn-bg p-4 text-xs text-warn-text">
                <p className="font-bold">Already on file.</p>
                <p className="mt-1">
                  Household head: {existingRecord.household_head || '—'} · Barangay: {existingRecord.barangay || '—'} ·
                  Last item: {existingRecord.item || '—'} ({new Date(existingRecord.created_at).toLocaleDateString()})
                </p>
                <button onClick={startNewWithSerial} className="mt-2 font-bold underline">
                  Continue anyway and record a new distribution for this serial number
                </button>
              </div>
            )}

            {serialChecked && !existingRecord && (
              <p className="mt-3 text-xs font-semibold text-donor-dark">No record found for this serial number — you may proceed to fill up the form below.</p>
            )}
          </Card>
          ...
          <Card className="rounded-none border-0 border-b border-line p-6 shadow-none">
            ...
            <div className="grid grid-cols-1 gap-3 border border-line-soft p-3 sm:grid-cols-2">
              <Field label="Item distributed"><input className={inputClass} value={form.item} onChange={(e) => update('item', e.target.value)} /></Field>
              <Field label="Quantity"><input className={inputClass} value={form.quantity} onChange={(e) => update('quantity', e.target.value)} /></Field>
            </div>
          </Card>
          ...
          <Card className="rounded-none border-0 p-6 shadow-none">
            ...
            <Button
              variant="admin"
              disabled={!signature || !form.lastName || !form.firstName || !form.item}
            ...
            {status === 'success' && (
              <div className="mt-3 rounded border border-beneficiary-dark bg-beneficiary-light p-3">
                ...
                <p className="mt-1 text-xs text-beneficiary-dark">Serial number: <span className="font-bold">{serialNumber}</span> — write this on the beneficiary's physical FACED card copy.</p>
              </div>
            )}
